...
from distributed_xfold.xsmm_kernels.prototypes.GridSelfAttention import GrigSelfAttentionXSMM
...
class GatingAttention(nn.Module):
    ...
    def forward(self, q_data, m_data, bias, nonbatched_bias=torch.Tensor()):
        ...
        output = (
            torch.einsum("bqhc,hco->bqo", weighted_avg, self.output_w)
        )
        return output
...
class Net2(nn.Module):  # Second network containing optimized attention layer
    def __init__(self):
        ...
        self.attention = GrigSelfAttentionXSMM(
            num_head=N, a_dim=HS, m_dim=HS, output_dim=HS
        )  # Attention layer
...
net2.attention.query_w.data = query_w.to(torch.bfloat16).contiguous()
net2.attention.key_w.data = key_w.to(torch.bfloat16).contiguous()
net2.attention.value_w.data = value_w.to(torch.bfloat16).contiguous()
net2.attention.gating_w.data = gating_w.to(torch.bfloat16).contiguous()
# net2.attention.gating_b.data = gating_b
net2.attention.output_w.data = output_w.to(torch.bfloat16).contiguous()
