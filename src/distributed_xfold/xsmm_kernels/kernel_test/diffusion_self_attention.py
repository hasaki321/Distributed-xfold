...
# class GatingAttention(nn.Module):
    ...
    def forward(self, q_data, m_data, bias, nonbatched_bias=torch.Tensor()):
        ...
        # get query, key, value
        q = (torch.einsum("qa,ahc->qhc", q_data, self.query_w) + self.query_b) * self.key_dim ** (-0.5)
        k = torch.einsum("ka,ahc->khc", m_data, self.key_w)
        v = torch.einsum("ka,ahc->khc", m_data, self.value_w)

        logits = torch.einsum("qhc,khc->hqk", q, k) + bias
        ...
        gate_values = (
            torch.einsum("qc,chv->qhv", q_data, self.gating_w)
        )
...
q_data = torch.randn(S, HS, requires_grad=False)
bias = torch.randn(1, 1, S, requires_grad=False)
...
net2.attention.query_w.data = net1.attention.q_projection.weight.T.reshape(qkv_shape).contiguous()
net2.attention.query_b.data = net1.attention.q_projection.bias.reshape((1,) + qkv_shape[1:]).contiguous()
net2.attention.key_w.data = net1.attention.k_projection.weight.T.reshape(qkv_shape).contiguous()
net2.attention.value_w.data = net1.attention.v_projection.weight.T.reshape(qkv_shape).contiguous()
net2.attention.gating_w.data = net1.attention.gating_query.weight.T.reshape(qkv_shape).contiguous()

net3.attention.query_w.data = net1.attention.q_projection.weight.T.reshape(qkv_shape).contiguous()
net3.attention.query_b.data = net1.attention.q_projection.bias.reshape((1,) + qkv_shape[1:]).contiguous()
net3.attention.key_w.data = net1.attention.k_projection.weight.T.reshape(qkv_shape).contiguous()
net3.attention.value_w.data = net1.attention.v_projection.weight.T.reshape(qkv_shape).contiguous()
net3.attention.gating_w.data = net1.attention.gating_query.weight.T.reshape(qkv_shape).contiguous()
# net2.attention.gating_b.data = gating_b


Y1 = net1(q_data, m_data, bias, nonbatched_bias)
Y2 = net2(q_data, (1e9 * (bias-1)), nonbatched_bias).flatten(-2, -1)
...
for _ in range(N):  # Optimized PyTorch layer Forward and Backward pass timing
    start = time.time()
    Y2 = net2(q_data, bias, nonbatched_bias)
