...
class BatchedSelfAttentionFunction(Function):
    @staticmethod
    def forward(
        ctx,
        q_data,
        ...
        result = torch.ops._alpha_attention.batch_diffusion_self_attention_forward(
            q_data.contiguous(),
            bias.contiguous(),
...
def BatchedSelfAttentionXSMM_forward(
    ...
    output = BatchedSelfAttentionFunction.apply(
        q_data,
        bias.float(),
...
class BatchedSelfAttentionXSMM(nn.Module):
