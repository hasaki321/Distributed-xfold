...
auto kfmask = -30000 * m_data.new_ones(K_t - Kp_t);
auto kfmask_a = GetVLAPtr<T>(kfmask, {1L});

auto q_data_a = GetVLAPtr<T>(q_data, {S_t, HS_t});
auto m_data_a = GetVLAPtr<T>(m_data, {K_t, HS_t});
// auto bias_a = GetVLAPtr<T>(bias, {S_t});
auto batched_bias_a = GetVLAPtr<T>(batched_bias, {N_t, S_t, K_t});
...
auto q_addbias_tpp = SCOPEIT(AddBiasTPP<T>(QKV_BLOCKSIZE, N_t * H_t, N_t * H_t), BIAS);
...
#pragma omp parallel for collapse(2)
    for (int64_t i = 0; i < B_t; i++) {
      for (int64_t j = 0; j < K_t; j += QKV_BLOCKSIZE) {
        T tmp[QKV_BLOCKSIZE * N_t * H_t];
...
// std::print64_tf("111 q k v calculate passed\n");
lda = H_t;
ldb = A_BLOCKSIZE;
ldc = S_t;
...
// (batched_bias.size(0) > 0)
//     logits = at::add(logits, at::unsqueeze(batched_bias, 0)); /* [512, 8,
...
// if (S_t < 2048) 
{
  auto a_brgemm_tpp = SCOPEITGEMM((BrgemmTPP<T, T>(
      A_BLOCKSIZE, A_BLOCKSIZE, H_t, 0, 0, N_t * H_t, K_t, K_t, 0.0, 0, 1)));
  ...
  auto a_add_bbias_tpp =
      SCOPEIT((AddTPP<T, T>(A_BLOCKSIZE, K_t, K_t, K_t)), BIAS);

  // auto a_add_sfmask_tpp =
  //     SCOPEIT(AddBiasTPP<T>(A_BLOCKSIZE, S_t - Sp_t, K_t), BIAS);
  auto a_add_kfmask_tpp =
      SCOPEIT(AddBiasTPP<T>(A_BLOCKSIZE, K_t - Kp_t, K_t), BIAS);
  auto a_softmax_tpp =
      ...
      for (int64_t i = 0; i < B_t; i++) {
        for (int64_t n = 0; n < N_t; n++) {
          for (int64_t j1 = 0; j1 < S_t; j1 += A_BLOCKSIZE) {
            ...
            T tmp_logits[A_BLOCKSIZE][K_t];

            for (int64_t j2 = 0; j2 < K_t; j2 += A_BLOCKSIZE) {
              a_brgemm_tpp(
                  &q_a[i][j1][n][0],
                  &k_a[i][n * H_t * K_t + j2], // 
                  &tmp_logits[0][j2],
...
auto g_sigmoid_tpp =
    SCOPEIT(SigmoidFwdTPP<T>(C_BLOCKSIZE, N_t* H_t, ldc, ldc), EW_MUL);
...
#pragma omp parallel for collapse(2)
    for (int64_t i = 0; i < B_t; i++) {
      for (int64_t j = 0; j < S_t; j += C_BLOCKSIZE) {
        ...
        g_sigmoid_tpp(&tmp[0], &tmp_gate_values[0]);
        g_mul_tpp(
            &tmp_gate_values[0],
...
// std::print64_tf("cal output passed\n");
// if (S_t != Sp_t) {
//   output = output.narrow(1, 0, Sp_t);
// }
// std::print64_tf("final narrow output passed\n");
