...
#pragma omp parallel for collapse(2)
    for (int64_t i = 0; i < B_t; i++) {
      for (int64_t j = 0; j < S_t; j += TRI_BLOCKSIZE) {
...
// for (int64_t i = 0; i < B_t; i++) {
#pragma omp parallel for collapse(2)
    for (int64_t i = 0; i < B_t; i += TRI_BLOCKSIZE) {
      for (int64_t j = 0; j < S_t; j += TRI_BLOCKSIZE) {
        ...
        for (int64_t ib = 0; ib < TRI_BLOCKSIZE; ib++) {
          proj_brgemm_tpp( &act_a[i + ib][j][0], &trans_proj_weight_a[0][0], &tmp[0][0], 1);
          // add_bias_tpp(&projection_bias_a[0][0], &tmp[0][0]);
          mul_broad_tpp(&mask_a[i + ib][j][0], &tmp[0][0], &tmp_proj[0][0]);

          proj_brgemm_tpp(&act_a[i + ib][j][0], &trans_gate_weight_a[0][0], &tmp[0][0], 1);
...
#pragma omp parallel for collapse(3)
      for (int64_t a = 0; a < num_intermediate_channel; a++) {
        for (int64_t i = 0; i < B_t; i += TRI_BLOCKSIZE) {
          for (int64_t j = 0; j < B_t; j += TRI_BLOCKSIZE) {
            ...
            for (int64_t ib = 0; ib < S_t / TRI_BLOCKSIZE; ib++)
              equation_trans_tpp(
...
#pragma omp parallel for collapse(3)
      for (int64_t a = 0; a < num_intermediate_channel; a++) {
        for (int64_t i = 0; i < S_t; i += TRI_BLOCKSIZE) {
          for (int64_t j = 0; j < S_t; j += TRI_BLOCKSIZE) {
            // equation_brgemm_tpp(&left_proj_act_a[a][i][0],
