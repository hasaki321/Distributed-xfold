...
auto q = q_data.new_empty({S_t, N_t, H_t}); /* [512, 764, 8, 32] */
auto q_a = GetVLAPtr<T>(q, {N_t, H_t});

auto k = q_data.new_empty({S_t* N_t* H_t}); /* [512, 764, 8, 32] */
auto k_a = GetVLAPtr<T>(k, {N_t * H_t});

auto v = q_data.new_empty({S_t* N_t* H_t}); /* [512, 764, 8, 32] */
auto v_a = GetVLAPtr<T>(v, {N_t * H_t});

auto weighted_avg =
    q_data.new_empty({S_t, N_t, H_t}); /* [512, 764, 8, 32] */
...
#pragma omp for
      // for (int i = 0; i < B_t; i++) {
        for (int j = 0; j < S_t; j += QKV_BLOCKSIZE) {
          ...
          q_convert_tpp(&tmp[0][0][0], &q_a[j][0][0]);
        }
      // }
...
// auto k = at::einsum("bka,ahc->bkhc", {q_data, key_w});
...
#pragma omp for
      // for (int i = 0; i < B_t; i++) {
        for (int j = 0; j < S_t; j += QKV_BLOCKSIZE) {
          ...
          kv_brgemm_tpp(
              &q_data_a[j][0], &qkv_w_vnni_a[0][0][0], &tmp[0], 1, true);
          k_trans_tpp(&tmp[0], &k_a[2 * j]);
        }
      // }
...
// auto v = at::einsum("bka,ahc->bkhc", {q_data, value_w});
// /* [512, 764, 8, 32]  = [512, 764, 256] * [256, 8, 32] */
{
...
#pragma omp for
      // for (int i = 0; i < B_t; i++) {
        for (int j = 0; j < S_t; j += QKV_BLOCKSIZE) {
          ...
          kv_brgemm_tpp(
              &q_data_a[j][0], &qkv_w_vnni_a[0][0][0], &tmp[0], 1, true);
          v_vnni_trans_tpp(&tmp[0], &v_a[j * N_t * H_t]);
        }
      // }
      kv_brgemm_tpp.release();
    }
...
#pragma omp for
      // for (int i = 0; i < B_t; i++) {
        for (int j = 0; j < S_t; j += C_BLOCKSIZE) {
          ...
          g_mul_tpp(&tmp_bf16[0], &weighted_avg_a[j][0][0], &output_a[j][0]);
