...
int64_t Sp_t = q_data.size(0); /* Query (764) */
int64_t HS_t = q_data.size(1); /* Channels (256) */
...
if (Sp_t % QKV_BLOCKSIZE != 0) {
  ...
  auto q_data_pad = q_data.new_zeros({S_t - Sp_t, HS_t});
  auto bias_pad = bias.new_zeros({1, 1, S_t - Sp_t});
  ...
  q_data = at::cat({q_data, q_data_pad}, 0);
  bias = at::cat({bias, bias_pad}, 2);
  if (flag) {
...
auto q_data_a = GetVLAPtr<T>(q_data, {HS_t});
...
auto q = q_data.new_empty({S_t, N_t, H_t}); /* [764, 8, 32] */
auto q_a = GetVLAPtr<T>(q, {N_t, H_t});

auto k = q_data.new_empty({S_t* N_t* H_t}); /* [764, 8, 32] */
auto k_a = GetVLAPtr<T>(k, {N_t * H_t});

auto v = q_data.new_empty({S_t, N_t, H_t}); /* [764, 8, 32] */
auto v_a = GetVLAPtr<T>(v, {N_t, H_t});

auto weighted_avg =
    q_data.new_empty({S_t, N_t, H_t}); /* [764, 8, 32] */
auto weighted_avg_a = GetVLAPtr<T>(weighted_avg, {N_t, H_t});

auto output = q_data.new_empty({S_t, N_t, H_t}); /* [512, 764, 256] */
auto output_a = GetVLAPtr<T>(output, {N_t, H_t});
...
#pragma omp parallel for
    // for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKV_BLOCKSIZE) {
        ...
        qkv_brgemm_tpp(&q_data_a[j][0], &query_w_a[0][0][0], &tmp[0], 1);
        q_addbias_tpp(&query_b_a[0][0], &tmp[0]);
        scale_tpp(&tmp[0], &q_a[j][0][0], alpha);
      }
    // }
  }
}
...
#pragma omp parallel for
    // for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKV_BLOCKSIZE) {
        T tmp[QKV_BLOCKSIZE * N_t * H_t];
        qkv_brgemm_tpp(&q_data_a[j][0], &key_w_a[0][0][0], &tmp[0], 1);
        k_trans_tpp(&tmp[0], &k_a[j]); // [ 0*H_t*S_t + 0*S_t + j]
      }
    // }
  }
}
...
#pragma omp parallel for
    // for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKV_BLOCKSIZE) {
        qkv_brgemm_tpp(
            &q_data_a[j][0], &value_w_a[0][0][0], &v_a[j][0][0], 1);
      }
    // }
  }
}
      ...
      // for (int i = 0; i < B_t; i++) {
        for (int n = 0; n < N_t; n++) {
          for (int j1 = 0; j1 < S_t; j1 += A_BLOCKSIZE) {
            ...
            a_addbias_tpp(&bias_a[0], &tmp_logits[0][0]);
            if (flag)
              a_add_nbbias_tpp(
                  &nonbatched_bias_a[n][j1][0],
                  &tmp_logits[0][0],
                  &tmp_logits[0][0]);
            ...
            c_brgemm_tpp(
                ...
                &tmp_o[0],
                S_t / A_BLOCKSIZE);
            a_cpy_tpp(&tmp_o[0], &weighted_avg_a[j1][n][0]);
          }
        }
      // }
    }
        ...
        for (int n = 0; n < N_t; n++) {
          for (int j1 = 0; j1 < S_t; j1 += A_BLOCKSIZE) {
            ...
            for (int j2 = 0; j2 < (S_t / Ak_BLOCKSIZE) * Ak_BLOCKSIZE;
                 j2 += Ak_BLOCKSIZE) {
              a_brgemm_tpp(
                  &q_a[j1][n][0], &k_a[n * H_t * S_t + j2], tmp_S, 1);

              a_addbias_online_tpp(&bias_a[j2], tmp_S);
              if (flag)
              ...
              c_brgemm_tpp(tmp_S, &v_a[j2][n][0], tmp_o1,
                           1); // O = P*V
              if (j2 == 0) {
            ...
            if (S_t % Ak_BLOCKSIZE != 0) {
              ...
              a_addbias_online_edge_tpp(&bias_a[j2], tmp_S_edge);
              if (flag) {
              ...
              c_brgemm_edge_tpp(tmp_S_edge, &v_a[j2][n][0], tmp_o1, 1);
              a_softmax_fixup_online(tmp_o1, tmp_o2, cmax, csum, omax, osum);
              delete[] tmp_S_edge;
            }

            a_softmax_scale_online(&tmp_o2[0], osum);
            a_cpy_tpp(&tmp_o2[0], &weighted_avg_a[j1][n][0]);
          }
        }
      // }
    }
...
#pragma omp parallel for
    // for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += C_BLOCKSIZE) {
        ...
        g_brgemm_tpp(&q_data_a[j][0], &gating_w_a[0][0][0], &tmp[0], 1);

        g_sigmoid_tpp(&tmp[0], &tmp[0], &tmp_gate_values[0]);
        g_mul_tpp(
            &tmp_gate_values[0],
            &weighted_avg_a[j][0][0],
            &output_a[j][0][0]);
      }
    // }
  }
